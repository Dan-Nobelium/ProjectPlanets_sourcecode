...
jsPsych.plugins['survey-likert-catch'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '.jspsych-survey-likert-statement { display: block; font-size: 16px; padding-top: 40px; margin-bottom: 10px; }'+
      '.jspsych-survey-likert-opts { list-style: none; width: ' + scale_width + '; margin: auto; padding: 0 0 35px; display: block; font-size: 14px; line-height: 1.1em; }'+
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      var options_string = '<ul class="jspsych-survey-likert-opts" data-name="' + question.name + '" data-radio-group="Q' + question_index + '">';
      ...
      if (question.required) {
        html += '<div class="jspsych-survey-likert-required-warning" style="display: none; color: red;">Please answer this question before continuing.</div>';
      }
    }
    ...
    display_element.querySelector('#jspsych-survey-likert-form').addEventListener('submit', function(event) {
      ...
      for (var index = 0; index < matches.length; index++) {
        var question_index = parseInt(matches[index].dataset['radioGroup'].replace('Q', ''));
        var question = trial.questions[question_index];
        var el = display_element.querySelector('input[name="Q' + question_index + '"]:checked');
  
        // Check if the question is required and unanswered
        if (question.required && el === null) {
          required_failed = true;
          display_element.querySelector('#jspsych-survey-likert-form .jspsych-survey-likert-opts[data-radio-group="Q' + question_index + '"]').nextElementSibling.style.display = 'block';
        } else {
          if (el === null) {
            var response = '';
          } else {
          ...
          var response_object = {};
          var name = (question.name !== '') ? question.name : 'Q' + question_index;
