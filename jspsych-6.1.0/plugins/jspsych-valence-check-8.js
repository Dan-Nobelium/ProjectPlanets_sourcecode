jsPsych.plugins['valence-check-8'] = (function () {
  ...
  plugin.trial = function (displayElement, trial) {
    ...
    let html = '<div id="jspsych-valence-check-8-wrapper" style="margin: 100px 0px;">';
    let startTime;
...
`;
    ...
    if (trial.require_movement) {
      document.querySelector(`#${inputIdBase}`).addEventListener("change", function () {
        document.querySelector("#jspsych-valence-check-8-next").disabled = false;
      });
    }

    // Event Listener For Click On The Submit Button
    document.querySelector("#jspsych-valence-check-8-next").addEventListener("click", function () {
      // Calculate RT
      const endTime = performance.now();
      response.rt = endTime - startTime;

      // Get Responses From All Eight Sliders
      response.val_1 = document.querySelector("#jspsych-valence-check-8-response1").value;
      response.val_2 = document.querySelector("#jspsych-valence-check-8-response2").value;
      response.val_3 = document.querySelector("#jspsych-valence-check-8-response3").value;
      response.val_4 = document.querySelector("#jspsych-valence-check-8-response4").value;
      response.val_5 = document.querySelector("#jspsych-valence-check-8-response5").value;
      response.val_6 = document.querySelector("#jspsych-valence-check-8-response6").value;
      response.val_7 = document.querySelector("#jspsych-valence-check-8-response7").value;
      response.val_8 = document.querySelector("#jspsych-valence-check-8-response8").value;

      // End Trial Or Disable The Submit Button Based On Settings
      if (trial.response_ends_trial) {
        end_trial();
      } else {
        document.querySelector("#jspsych-valence-check-8-next").disabled = true;
      }
    });
