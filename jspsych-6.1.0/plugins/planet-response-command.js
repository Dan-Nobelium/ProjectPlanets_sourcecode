...
plugin.trial = function(display_element, trial) 
{
    ...
    planetContainer.style.flex = '2'; // Left 2/3 of the screen
    ...
    infoContainer.style.flex = '1'; // Right 1/3 of the screen
    infoContainer.style.display = 'flex';
    infoContainer.style.flexDirection = 'column';
    infoContainer.style.alignItems = 'center';
    infoContainer.style.justifyContent = 'space-around';

    // Create info elements and add them to the info container
    var totalScoreBox = document.createElement('div');
    totalScoreBox.id = 'total-score-box';
    infoContainer.appendChild(totalScoreBox);

    var shipImgBox = document.createElement('div');
    shipImgBox.id = 'ship-img-box';
    infoContainer.appendChild(shipImgBox);

    var shipShieldBox = document.createElement('div');
    shipShieldBox.id = 'ship-shield-box';
    infoContainer.appendChild(shipShieldBox);

    // Append the planet container and info container to the main container
    container.appendChild(planetContainer);
    container.appendChild(infoContainer);

    // Append the main container to the display element
    ...
    shipImgBox.innerHTML = `
        <div id="ship-img-div" style="position:relative; top:0px; border: 0px; height: ${trial.ship_height}px; width: ${trial.ship_width}px;" draggable="false"></div>
    ...
    var tradeProbArrs, trade_orderbase, trade_log, trade_read;
    if (trial.trade_balance) {
        tradeProbArrs = initProbArray(trial.probability_trade);
        trade_orderbase = tradeProbArrs[0];  // Base set to randomize availability of trades
        trade_log = tradeProbArrs[1]; // Arr to read future trade success
        trade_read = tradeProbArrs[2]; // Arr to log trade success
    }

    var shipProbArrs, ship_orderbase, ship_log, ship_read;
    if (trial.ship_balance) {
        shipProbArrs = initProbArray(trial.probability_ship);
        ship_orderbase = shipProbArrs[0];
        ship_log = shipProbArrs[1]; 
        ship_read = shipProbArrs[2];
    }

    var shieldProbArrs, shield_orderbase, shield_log, shield_read;
    if (trial.shield_balance) {
        shieldProbArrs = initProbArray(trial.probability_shield);
        shield_orderbase = shieldProbArrs[0]; 
        shield_log = shieldProbArrs[1]; 
        shield_read = shieldProbArrs[2]; 
    }
    ...
    for (var i = 0; i < trial.stimulus.length; i++) {
        ...
        var conditionStr = 'element.getAttribute("allowclick")=="1" && shield_activated!=true';
        ...
        var elementbx = display_element.querySelector('#planet-score-' + i);
    ...
    function after_response(element) {
        ...
        // Since response.clicks.idx is updated only after this script, add 1 to the number
        if (click_idx == null) {
            click_idx = 0;
        } else {
            click_idx++;
    ...
    function proceed_trade(choice) {
        ...
        document.querySelector('#planet-signal-' + choice).innerHTML = `
            <img src="img/signal1.png" id="planet-signal-img-${choice}" style="display:block; position: relative; height: ${trial.signal_width - 10}px; width: ${trial.signal_width}px; left: ${signalLeft}px; padding: ${signalPadding}px; visibility: visible;">
        `;
        ...
        function sigframe() {
            ...
            if (curr_time > signal_max_time) {
                ...
                var dots = colordots(signal_dot_count_max, signal_dot_count, 'black', signalclr);
                signal_dot_count++;
        ...
        function colordots(totalct, colorct, baseclr, fontclr) {
            ...
            for (var i = 0; i < totalct; i++) {
                var color = (i < colorct) ? fontclr : baseclr;
        ...
        var displayScore, statusmsg, statusclr;
        if (trade_success) {
            // Add and display reward
            displayScore = trial.rewards[choice];
            statusmsg = 'Success! <br> <b>+' + displayScore + ' points </b>';
            statusclr = '#05BF00'; // some shade of green
        } else {
        ...
        if (trial.ship_balance) {
            ...
            show_ship_check = (show_ship_samp < trial.probability_ship[choice]);
        }
    ...
    // Function to show ship img and subsequent procedure
        ...
        shipDiv.innerHTML = `
            <img src="${trial.ship_stimulus[choice]}" id="ship-img" class="ship" height="${trial.ship_height}" width="${trial.ship_width}" style="position:relative; top:0px; border: 0px; visibility:visible; z-index:11;" draggable="false">
        `;
        shipVisible = true;
        ...
        shieldBoxDiv.innerHTML = `
            <div class="ship" id="ship-shield-text"></div>
        ...
        shieldChgDiv.style.opacity = 0.5;
    ...
    function ship_attack(choice) {
        ...
        if (!shield_activated) {
            ...
            // Update status
            statusmsg = 'Ship engaged for: <b>-' + appliedDamage + ' points</b>';
            statusclr = 'grey'; // some shade of red - optionally make this green/orange/red for tiers of damage for each damage level
        } else if (shield_activated) {
            statusmsg = 'Shield successfully deflected attack';
            statusclr = '#05BF00';
        }
    ...
    function updateScore(points) {
        ...
        if (trial.show_total_points) {
            var scoreDiv = display_element.querySelector('#total-score-box');
    ...
    function updateStatus(choice, msg, color) {
        // Update planet status with some message and in some color
        var statusDiv = (choice === 'ship') ? display_element.querySelector('#ship-status-text') : display_element.querySelector('#planet-score-' + choice);
...
// Track mouse events
    ...
    function log_click(cursor_loc) {
        ...
        display_element.style.cursor = "url('" + trial.cursor[1] + "'),pointer";
    }
    ...
    function check_end() {
        ...
        var checkStr = [];
        for (var i = 0; i < trial.stimulus.length; i++) {
            var el = display_element.querySelector('#planet-score-' + i);
            if (el.innerHTML !== '') {
                checkStr.push('Planet ' + i + ' ' + el.innerHTML);
                check_count++;
            }
        }
        var checkPlanet = (check_count === 0);

        var checkShip = true;
        if (trial.show_ship) {
            checkShip = !shipVisible;
        }
...
function balanceSuccess(orderbase, arr_log, arr_read, choice, verbose = false, vstr = '') {
    ...
    var arr_success = Boolean(arr_read[choice][arr_next]);
    if (verbose) {
...
// Initialize variables for balanced probability arrays
