jsPsych.plugins['valence-check-5'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '<div id="jspsych-valence-check-5-stimulus">';
    ...
    html += '">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-valence-check-5-response3"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels.length; j++){
      var width = 100/(trial.labels.length-1);
      var left_offset = (j * (100 /(trial.labels.length - 1))) - (width/2);
    ...
    html += '">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-valence-check-5-response4"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels.length; j++){
      var width = 100/(trial.labels.length-1);
      var left_offset = (j * (100 /(trial.labels.length - 1))) - (width/2);
    ...
    html += '<div id="jspsych-valence-check-5-stimulus">';
    html += '<img src="'+trial.stimulus_5+'" style="';
    if(trial.stimulus_height !== null){
      html += 'height:'+trial.stimulus_height+'px; '
      if(trial.stimulus_width == null && trial.maintain_aspect_ratio){
        html += 'width: auto; ';
    ...
    // stimulus text
    if (trial.stim_text_5 !== null){
      html += trial.stim_text_5;
    }
    html += '<br><br>';
    ...
    html += '">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-valence-check-5-response5"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels.length; j++){
      var width = 100/(trial.labels.length-1);
      var left_offset = (j * (100 /(trial.labels.length - 1))) - (width/2);
    ...
    if(trial.require_movement){
      display_element.querySelector('#jspsych-valence-check-5-response5').addEventListener('change', function(){
        display_element.querySelector('#jspsych-valence-check-5-next').disabled = false;
      })
    }
    ...
    display_element.querySelector('#jspsych-valence-check-5-next').addEventListener('click', function() {
      ...
      response.response1 = display_element.querySelector('#jspsych-valence-check-5-response1').value;
      response.response2 = display_element.querySelector('#jspsych-valence-check-5-response2').value;
      response.response3 = display_element.querySelector('#jspsych-valence-check-5-response3').value;
      response.response4 = display_element.querySelector('#jspsych-valence-check-5-response4').value;
      response.response5 = display_element.querySelector('#jspsych-valence-check-5-response5').value;

      if(trial.response_ends_trial){
        end_trial();
      } else {
        display_element.querySelector('#jspsych-valence-check-5-next').disabled = true;
      }
    ...
    function end_trial(){
      ...
      var trialdata = {
        ...
        "val_4": response.response4,
        "val_5": response.response5
      };
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-valence-check-5-stimulus').style.visibility = 'hidden';
